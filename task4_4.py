...
def poly(nameFile):
    openFile = open(nameFile, mode='r', encoding='utf8')
...
format = 'Результат: {0}x^2+{1}x+{2} '.format(result[0],result[1],result[2]).replace('+-', '-').replace('0x^2', '').replace('1x^2', 'x^2').replace('+0x', '').replace('1x', 'x').replace('+0', '')
